...
Коэффициент заполнения — это отношение числа элементов в хэш-таблице к количеству ячеек массива. С ростом коэффициента увеличивается вероятность коллизий, что снижает производительность. **В учебных целях** в проекте выбран коэффициент заполнения **15**, обеспечивающий множественные узкие места для оптимизаций. 

При превышении этого значения размер таблицы может автоматически увеличиваться в 2 раза, и данные в таком случае будут перехэшированы. Такой функционал достигается
при использовании функции `fillHashTableWithRehash` для заполнения таблицы.

Если же пользователь уверен в аккуратном и правильном использовании таблицы и способности самостоятельно контролировать ее состояние,
реализованы функции `fillHashTable` и `rehashTable`.
...
При коэффициенте заполнения 15 функция **`CRC32`** показала лучшую дисперсию (17.01), обеспечивая оптимальное распределение данных. 
...
где:
- $T_{\text{prev}}$ — время предыдущей версии,
- $T_{\text{curr}}$ — время текущей версии,
- $T_0$ — время неоптимизированной версии.

> [!NOTE]  
> Тесты проводились на Intel Core i5-1235U (12 ядер, 16 потоков, 10 нм, 1.3 ГГц, буст до 4.4 ГГц) Для стабильной работы процессора режим `Turbo Boost` во время тестирования был отключен.

Перед тестированием каждой хэш-функции прогреваются кэши.
Выполняется `100` тестов по `100000` поисков в хэш-таблице, полученные значения времени усредняются.

### Базовые результаты
Сравнение времени работы хэш-функций с флагами компиляции `-O0` и `-O3`:

| Хэш-функция | Время (-O0, такты) | Среднее отклонение времени (такты) |
|-------------|--------------------| ---------------------------------- |
| PJW32       | 1360855            |  45588.64                          |                                    
| Adler32     | 1486180            |  29872.21                          |
| SDBM32      | 1472729            |  27540.03                          |
| FNV32       | 1369590            |  57385.82                          |
| Murmur3     | 1440013            |  29088.26                          |
| CRC32       | 4773105            |  134601.56                         |

| Хэш-функция | Время (-O3, такты) | Среднее отклонение времени (такты) |
|-------------|--------------------| ---------------------------------- |
| PJW32       | 642266             |  17854.99                          |                                    
| Adler32     | 770131             |  15248.59                          |
| SDBM32      | 694611             |  14864.67                          |
| FNV32       | 691238             |  17419.19                          |
| Murmur3     | 645002             |  11481.03                          |
| CRC32       | 1650262            |  33170.26                          |
...
С оптимизациями компилятора `-O3` наибольшой прирост производительности получился при использовании хэш-функции crc32.
Проанализируем оба бинарных файла с помощью [Ghidra](https://github.com/NationalSecurityAgency/ghidra).

* `-O0`
Код с четкой структурой циклов для обработки 8 итераций побитового сдвига на байт. Используется много переменных на стеке, код очень близок к исходному по своей структуре.
Обнаруживаются вложенные циклы с явными инструкциями ветвления (`jmp`, `jbe`, `jb`)

<p align="center">
  <a href="" rel="noopener">
 <img src="https://i.ibb.co/3wn824J/image-2025-05-06-06-02-46.png" alt="O0"></a>
</p>

* `-O3`
Код компактнее, внутренний цикл (`L4` в `-O0`) полностью развернут в последовательность операций, что устраняет накладные расходы
на управление циклом (инкремент счетчика, сравнение, переход). Активно используются регистры, минизируя доступ к памяти.
Есть обработка случая с нулевой длиной входных данных (`test rsi, rsi; je .L4`), что оптимизирует граничные случаи.
Вероятно, компилятор проанализировал зависимости данных в алгоритме и оптимизировал порядок инструкций для избежания простоев - повторяющиеся операции
`and`, `neg`, `shr`, `xor` организованы так, чтобы максимизировать эффективность конвейера процессора.

<p align="center">
  <a href="" rel="noopener">
 <img src="https://i.ibb.co/gZzSKcH7/O3.png" alt="O3"></a>
</p>

Для дальнейшей оптимизации работы хэш-таблицы остановимся на этой хэш-функции, так как в ней еще есть большой потенциал для оптимизаций.

> [!NOTE]
> В последующих таблицах с измерениями будут указываться результаты работ всех хэш-функций, однако именно на следующем этапе не стоит
> обращать большое внимание на работу других хэш-функций. Влияние других оптимизаций на работу хэш-функций можно изучить в разделах
> `Оптимизация функции strcmp с помощью Intel Intrinsics` и `Оптимизация создания и инициализации ячейки в hash-bucket на языке assembler x86-64 (NASM)`
...
Попробуем использовать инструкции `_mm_crc32_uXX` для распараллеливания и ускорения вычислений. Они позволят обрабатывать 8, 16, 32 и 64 бит бита
за один цикл. Будем ожидать хорошего прироста, но стоит заметить, что на больших размерах обрабатываемых буферов данных прирост будет намного заметнее.
Теперь новая версия функции выглядит так:
...
Среднее время работы хэш-таблицы сократилось с 1650262 до 657693 тактов. 

Результаты:

| Хэш-функция | Время (-O3 + crc32 Intrinsics, такты) | Среднее отклонение времени (такты) |
|-------------|---------------------------------------| ---------------------------------- |
| PJW32       | 653341                                |  11629.46                          |                                    
| Adler32     | 772306                                |  15523.35                          |
| SDBM32      | 695698                                |  16140.19                          |
| FNV32       | 683368                                |  13394.01                          |
| Murmur3     | 662906                                |  10937.94                          |
| CRC32       | 657693                                |  10128.47                          |
...
Относительный прирост более чем в два раза - отличный результат! Не будем останавливаться на достигнутом.

#### Оптимизация функции strcmp с помощью Intel Intrinsics
После оптимизации `crc32` узким местом стала функция `strcmp`:
...
Конкретно в нашем случае отсутствие обработки краевых случаев и использование невыровненной загрузки может в среднем давать совсем небольшой прирост производительности
по сравнению со стандартной реализацией `strcmp` в `libc`. Однако стоит учитывать, что эта реализация является менее универсальной и вовсе не безопасной.

Результаты:

| Хэш-функция | Время (-O3 + crc32 Intrinsics + strcmp Intrinrics, такты) | Среднее отклонение времени (такты) |
|-------------|-----------------------------------------------------------| ---------------------------------- |
| PJW32       | 652275                                                    |  27591.23                          |
| Adler32     | 770290                                                    |  17023.40                          |
| SDBM32      | 694486                                                    |  15973.17                          |
| FNV32       | 691994                                                    |  16953.85                          |
| Murmur3     | 657496                                                    |  21828.86                          |
| CRC32       | 645848                                                    |  14079.48                          |
...
| Хэш-функция | Время (-O3 + crc32 Intrinsics + strcmp Intrinsics + createNode ASM, такты) | Среднее отклонение времени (такты) |
|-------------|----------------------------------------------------------------------------| ---------------------------------- |
| PJW32       | 625838                                                                     | 9637.90                            |
| Adler32     | 712821                                                                     | 9409.23                            |
| SDBM32      | 621773                                                                     | 11067.55                           |
| FNV32       | 609514                                                                     | 12068.37                           |
| Murmur3     | 581884                                                                     | 8964.01                            |
| CRC32       | 553139                                                                     | 7275.03                            |
...
| Хэш-функция | Время (-O3 + crc32 Intrinsics + strcmp Intrinsics + createNode ASM + adler32 ASM, такты) | Среднее отклонение времени (такты) |
|-------------|------------------------------------------------------------------------------------------| ---------------------------------- |
| PJW32       | ...                                                                                      | ...                                |
| Adler32     | 599227                                                                                   | 8568.94                            |
| SDBM32      | ...                                                                                      | ...                                |
| FNV32       | ...                                                                                      | ...                                |
| Murmur3     | ...                                                                                      | ...                                |
| CRC32       | ...                                                                                      | ...                                |
...
в неоптимизированной версии. Используется инструкция процессора для распаковки 32-битного слова на отдельные байты с помощью побайтовых операций (`movzbl` и сдвигов `shr`).
Модуль (`%65521`) применяется только после обработки всех 4-байтовых блоков, в цикле для оставшихся байтов, что значительно сокращает количество операций деления, которые 
являются достаточно дорогими.
Прямое управление регистрами минимизирует накладные расходы при работе со входными данными.
...
Результаты зависят от реализации и платформы. Платформозависимые оптимизации эффективны, но снижают переносимость кода и усложняют поддержку. Их применение следует оценивать с учетом поставленных задач.
