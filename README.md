...
Выполняется `100` тестов по `1000` поисков тестового словаря размером в половину исходного (примерно 9650 слов) в хэш-таблице, полученные значения времени усредняются.
...
Сделаем небольшую ассемблерную вставку внутри функции `searchString`:
...
Видим относительный прирост на 32% в реализации с хэш-функцией `crc32` и 28% - с `adler32`. Кроме того, уменьшились накладные расходы на вызов функции `strcmp`.
